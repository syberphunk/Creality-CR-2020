...
static void lcd_filament_change()
{
	START_MENU();
	MENU_ITEM(back, msg_prepare(), lcd_prepare_menu);
	MENU_ITEM(function, msg_load(), lcd_filament_load);
	MENU_ITEM(function, msg_unload(), lcd_filament_unload);
	END_MENU();
}
...
void next()
{
  ...
  { 
    ...
    enqueuecommands_P((PSTR("G1 X20 Y20 F5000"))); // Coordinated Movement X:20mm Y:20mm
  ...
  if(i==2)
  {
    ...
    enqueuecommands_P((PSTR("G1 Y180 F5000"))); // Coordinated Movement X:20mm Y:180mm
    enqueuecommands_P((PSTR("G1 F200 Z-3"))); // Coordinated Movement Z -3mm
  }
  if(i==3)
  {
    enqueuecommands_P((PSTR("G1 F200 Z3"))); // Coordinated Movement Z 3mm
    enqueuecommands_P((PSTR("G1 X180 F5000"))); // Coordinated Movement X:180mm Y:180mm
    enqueuecommands_P((PSTR("G1 F200 Z-3"))); // Coordinated Movement Z -3mm
  }
  if(i==4)
  {
    enqueuecommands_P((PSTR("G1 F200 Z3"))); // Coordinated Movement Z 3mm
    enqueuecommands_P((PSTR("G1 Y20 F5000"))); // Coordinated Movement X:180mm Y:20mm
    enqueuecommands_P((PSTR("G1 F200 Z-3"))); // Coordinated Movement Z -3mm
  }
  if(i==5)
  {
